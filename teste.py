...
def find_best_threshold(algorithm, extracted_ingredients_amostra, cleaned_allergens_set, allergen_mapping, df_teste):
    best_threshold = 0
    best_f1 = 0
    for threshold in range(0, 101):
        ...
        df_teste[f'alergenos_{algorithm.value}'] = detected_allergens.apply(lambda x: [detected[0] for detected in x])
...
def load_and_preprocess_sample():
    df_teste = pd.read_csv('conjunto_teste.csv', sep="\t", low_memory=False,
                           usecols=["code", "product_name_pt", "ingredients_text_pt", "gabarito"])

    df_teste = preprocess_data(df_teste)

    extracted_ingredients_amostra = df_teste['ingredients_text_pt'].progress_apply(extract_ingredients)

    df_teste['gabarito'] = df_teste['gabarito'].apply(ast.literal_eval).apply(lambda x: [clean_text(i) for i in x])
    return df_teste, extracted_ingredients_amostra


def teste(extracted_ingredients_amostra, cleaned_allergens_set, allergen_mapping, df_teste):
